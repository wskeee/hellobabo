...
class Coupon extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['used', 'type', 'with_special', 'with_id', 'with_amount', 'used_amount', 'quota', 'take_count', 'used_count', 'valid_type', 'valid_days', 'status', 'created_by', 'updated_by', 'created_at', 'updated_at'], 'integer'],
            [['title', 'start_time', 'end_time', 'valid_start_time', 'valid_end_time',], 'string', 'max' => 64],
            [['icon_url', 'des'], 'string', 'max' => 255],
            ['start_time', 'filter', 'filter' => function () {
                return empty($this->start_time) ? 0 : strtotime($this->start_time . ' 00:00:00');
            }],
            ['end_time', 'filter', 'filter' => function () {
                return empty($this->end_time) ? 0 : strtotime($this->end_time . ' 23:59:59');
            }],
            ['valid_start_time', 'filter', 'filter' => function () {
                return empty($this->valid_start_time) ? 0 : strtotime($this->valid_start_time . ' 00:00:00');
            }],
            ['valid_end_time', 'filter', 'filter' => function () {
                return empty($this->valid_end_time) ? 0 : strtotime($this->valid_end_time . ' 23:59:59');
            }],
        ];
    }
    ...
    {
        $this->start_time = $this->start_time == 0 ? '' : date('Y-m-d', $this->start_time);
        $this->end_time = $this->end_time == 0 ? '' : date('Y-m-d', $this->end_time);
        $this->valid_start_time = $this->valid_start_time == 0 ? '' : date('Y-m-d', $this->valid_start_time);
        $this->valid_end_time = $this->valid_end_time == 0 ? '' : date('Y-m-d', $this->valid_end_time);
